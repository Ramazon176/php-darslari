<?php
$x = 20;
function summa($summabek ){
    for($x = 1; $x % 2 == 0; $x++){
        echo "a";
    }
}
// aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
$sonlar =array(2,5,34,565,634,56,4356,4,56,4);
$sonlarbek=[2,5,34,565,634,56,4356,4,56,4];
print_r($sonlar);
echo $sonlar[2];
$cars =array("sgdkjh","gufyd");
var_dump($cars);
// bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
echo $sonlarbek[4];
echo "\n";
$sonlarbek[4] =102;
echo $sonlarbek[4];
print_r($sonlarbek);

// cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

$cars = array("pagani","ferrari","bugatti");
foreach($cars as $x);
echo $x;

// dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd


$user=["ism"=>ramazon,"familiya"=>vapoyev,"it"=>2011,];
print_r($user);
echo $user["ism"];
$user["familiya"]="karimov";
echo $user["familiya"];
$user["tel"] = 905796077;

// mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm





























































































































































































?>